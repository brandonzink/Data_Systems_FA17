...
$query = "SELECT items.id AS id,feedTitle,feedLink,itemTitle,itemPubDate,itemLink,itemDesc,imageURL FROM feeds,items WHERE feeds.displayColumn=1 AND feeds.id=items.id";
DisplayColumn($db, $query);

echo "</div>\n";
echo "<div id=\"content-middle\">\n";

$query = "SELECT items.id AS id,feedTitle,feedLink,itemTitle,itemPubDate,itemLink,itemDesc,imageURL FROM feeds,items WHERE feeds.displayColumn=2 AND feeds.id=items.id";
DisplayColumn($db, $query);

echo "</div>\n";
echo "<div id=\"content-right\">\n";

$query = "SELECT items.id AS id,feedTitle,feedLink,itemTitle,itemPubDate,itemLink,itemDesc,imageURL FROM feeds,items WHERE feeds.displayColumn=3 AND feeds.id=items.id";
DisplayColumn($db, $query);
...
function DisplayItem($prev, $item)
{
    ...
    echo "<img class='articleImage' src='".$item['imageURL']."'/>";
    echo "</article>\n";
}
